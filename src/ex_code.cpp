...
{
    // For Thunder Effect
    RectangleShape lightningOverlay(Vector2f(1920, 1080));  // Full-screen overlay
    lightningOverlay.setFillColor(Color(255, 255, 255, 0)); // Initially invisible
    bool lightningActive = false;
    float lightningDuration = 0.1f; // Short duration
    Clock lightningClock;

    // For the thunder sound
    ...
    window.setView(view);
    // End of the low res code!! Everything else is the same
    ...
    while (window.isOpen())
    {
        ...
        if (!lightningActive && rand() % 1500 == 0)
        { // Adjust probability
            lightningActive = true;
            lightningClock.restart();
            lightningOverlay.setFillColor(Color(255, 255, 255, 150)); // Bright flash
        }

        // Fade out lightning
        if (lightningActive)
        {
            float elapsed = lightningClock.getElapsedTime().asSeconds();
            if (elapsed > lightningDuration)
            {
                lightningOverlay.setFillColor(Color(255, 255, 255, 0)); // Fade out
                lightningActive = false;
            }
        }
        if (!lightningActive && rand() % 500 == 0)
        {
            lightningActive = true;
            lightningClock.restart();
            lightningOverlay.setFillColor(Color(255, 255, 255, 150));
        ...
        window.draw(lightningOverlay); // Draw lightning last
        window.display();
    }
